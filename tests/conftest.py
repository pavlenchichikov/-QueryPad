...
def isolated_ml(tmp_path, monkeypatch):
    ...
    data_dir.mkdir()
    monkeypatch.setattr(ml_local, "DATA_DIR", data_dir)
    monkeypatch.setattr(ml_local, "HISTORY_PATH", data_dir / "query_history.jsonl")
    monkeypatch.setattr(ml_local, "PATTERNS_PATH", data_dir / "learned_patterns.json")
    monkeypatch.setattr(ml_local, "STATS_PATH", data_dir / "model_stats.json")
    monkeypatch.setattr(ml_local, "_model", None)
    yield ml_local
    monkeypatch.setattr(ml_local, "_model", None)
